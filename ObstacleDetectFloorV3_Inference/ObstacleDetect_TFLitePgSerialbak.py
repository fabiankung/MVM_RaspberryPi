...
display.set_caption("Obstacle Detect Serial")
...
#sport.port = '/dev/serial0' # Tested ok for raspberry pi 3A+ with Rasbian OS Bullseye
sport.port = '/dev/ttyS0'  # Tested ok for raspberry pi 3A+ with Rasbian OS Bullseye
...
while is_running:
    ...
    # Send output to Robot Controller.
    #"R1x00", where x = '0','1',...'4'
    HeadRC_Interface.sendtoRC(bytes([82,49,48+result,48,48,10]))
    nTXErrorCnt = 0
    #while HeadRC_Interface.sendtoRC(bytes([82,49,48+result,48+result,48+result,10])) == False:
    #     nTXErrorCnt = nTXErrorCnt + 1
